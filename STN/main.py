...
from STN.load_data import Data
from STN.STNet import Transformer, STNet


resume = "model_stn.pth.tar"
start_epoch = 0
global_lr = 0.001
evaluate_chk = False
cuda_chk = True
...
print_freq = 10
if cuda_chk:
    resume = "./parameter/cuda_stn.pth.tar"
...
def main():
    ...
    if cuda_chk:
        ...
        #model.cuda()
        model.localisation_conv.cuda()
        model.localisation_fc.cuda()
        model.features.cuda()
        model.linear.cuda()
        for i in range(5):
            model.classification[i].cuda()
    ...
    for idx, (key, value) in enumerate(params_dict.items()):
        if idx < 10:
            params += [{'params': [value], 'lr': global_lr * 0.1}]
        else:
